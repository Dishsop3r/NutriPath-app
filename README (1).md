...
1. Front End  
   1. The Front End will be created in App Inventor by using its visual design tools to create the desired UI and then writting code for the app functionality.  
   2. The application will have a few UI screens: a Home screen, an Add Meal screen, a Daily Log, and a Progress/History screen. All screens will be simplified to the point where the user does not have to put in more information than necessary to use the app. All screens will follow a consistent UI/UX pattern to avoid causing confusion.  
2. Back End  
   1. The Back End will be a simple database to hold food entries and calorie totals. It will be implemented using the TinyDB block in App Inventor.   
   2. Each log entry will be an object containing food name, calories, nutrients, meal type, and entry date/time. There is an option to implement a cloud database (such as Firebase) to provide synchronization functionality should the application be ported to iOS.

# V. Functionality

1. Must Have Features  
   1. Logging of individual food entries with calories, name, and type.  
   2. Display of current day’s total calories and progress towards the daily goal.  
   3. Logging of individual food entries with calories, name, and type.  
   4. Meals can be categorized as breakfast, lunch, dinner, or others. Entries can be viewed per day or per week.  
2. Stretch Features  
   1. Option to set/edit the daily goal. The goal will be displayed prominently on the Home Screen.  
   2. Nutrients (protein/fat/carbs) can be tracked alongside calories.  
   3. On-screen progress indicator (progress bar) that displays how much of the daily goal has been achieved.
...
GitHub Repository: https://github.com/Dishsop3r/NutriPath-app.git

GitHub Wiki (Outline):  [https://docs.google.com/document/d/1pawdzs2a4yMhUJ1VHj89HaDHrP41bZGF0bNf3aW5Ihc/edit?usp=sharing](https://docs.google.com/document/d/1pawdzs2a4yMhUJ1VHj89HaDHrP41bZGF0bNf3aW5Ihc/edit?usp=sharing) 